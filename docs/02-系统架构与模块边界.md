...
第一版采用“顶层按业务能力划分、模块内部最多再分一层”的结构，避免所有文件堆在一个目录，也避免为了形式制造过深目录。
...
- 顶层目录按能力边界划分，例如 `api`、`rag`、`agent`、`database`，不按 Controller/Service/Util 机械复制 Java 包结构；
- 只有同一职责出现多个实现或多个稳定数据族时才建立子目录；
- 第一版目录深度通常不超过 `evidence_desk/<模块>/<子模块>/<文件>.py`；没有真实需求时不继续向下嵌套；
- 不提前创建空的 `application`、`agent`、`tools`、`evaluation` 子目录，任务开始时再创建；
- 不建立容易变成杂物箱的 `utils.py`、`helpers.py` 或全局 `common.py`；公共能力必须有明确归属；
- `rag/models/` 按数据生命周期分为 `source`、`cleaning`、`chunk`、`retrieval`，不是一个类一个文件，也不把全部模型堆进单个 `models.py`；
- `rag` 中的执行模块暂时保持扁平；只有真实出现多个实现时，才进一步拆分 `ingestion`、`indexing` 或 `retrieval` 子包。

当前阶段只创建正在使用的文件。例如 P1A-01 只创建 `models/source.py`，到 Cleaner、Chunk 和 Retrieval 阶段再分别增加对应模型文件。
